...
CROSSPOINT_PATH = '/root/turtlebot3_ws/src/traffic_topic/config/crossing_points/test.yaml'
...
class TrafficJudgmenter(Node):
    ...
    def traffic_judgment(self):
        ...
        if self.current_waypoint_msg is not None and self.traffic_msg is not None:
            ...
            if self.current_waypoint_msg in self.crossing_points_numbers and "person" in self.traffic_msg and not self.traffic_request:
                
                # flag_on
                self.traffic_request = True
                time.sleep(3)
                # サービスリクエストの送信
                self.send_request()
                time.sleep(2)
